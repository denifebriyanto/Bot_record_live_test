...
async def is_live(username: str):
    now = time.time()
    if username in _cache and now - _cache[username]["time"] < CACHE_TTL:
        cached = _cache[username]
        return cached["live"], cached["url"]
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=15), allow_redirects=True) as resp:
                ...
                if "LiveRoom" not in html and "liveRoom" not in html:
                    ...
                    _cache[username] = {"live": False, "url": None, "time": now}
                    return False, None

                match = re.search(r'(https://[^"\'\\]+\.m3u8[^"\'\\]*)', html)
                if match:
                    hls = match.group(1)
                    _cache[username] = {"live": True, "url": hls, "time": now}
                    return True, hls

                room_match = re.search(r'"roomId"\s*:\s*"?(\d+)"?', html)
                if not room_match:
                    _cache[username] = {"live": True, "url": None, "time": now}
